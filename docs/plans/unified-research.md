...
- [ ] **Dashboard history catch-up** (`fix/dashboard-catchup`): Browser QA found that request-paced 100,000-event batches leave the 2.36-million-event master history behind when catalog refreshes are cached for 30 seconds. Decouple bounded read-only catch-up from request cadence, preserve complete counters and identity/reset semantics, and test worker lifecycle/shutdown. Own `DashboardReader` and coordinate with the browser worker's HTTP additions.
