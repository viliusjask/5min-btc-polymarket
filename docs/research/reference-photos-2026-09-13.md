...
| `SvBga7VA.jpg` | Continuation of the same post | Rule: wait until about two minutes before the round closes; require a USD 70 to 100 BTC move in the round; buy with the move at 0.80 to 0.99; cover a small part only if the book becomes very imbalanced (about 95/5) | Historical: the `momentum` comparator implements a modified version; the literal rule is not registered | test as the literal "screenshot baseline" (B3 in the plan) |
| `5K48FChw.jpg` | Continuation: link to `github.com/Novals83/5min-...`, a PowerShell log, a quoted @MikuBTC video, dated 05 Sep 26, 164K views | Log rows show fills at 0.81 to 0.96 settling at 1.00, one "book flipped 95/5 -> micro-hedge -1.69" row | Historical: the upstream repository was audited; its tracked code cannot place the shown orders | reject as evidence; the log is not a trade export and the repository lacks the runner |
